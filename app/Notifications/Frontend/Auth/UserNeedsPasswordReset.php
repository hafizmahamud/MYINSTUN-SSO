...
class UserNeedsPasswordReset extends Notification
{
    ...
    {   $last_page = collect(request()->segments())->first();
        if($last_page == "activate")
        {
            return (new MailMessage())
                ->subject(app_name().': '.__('Activate Account'))
                ->line(__('You are receiving this email because we received an activation request for your account.'))
                ->action(__('Activate'), route('frontend.auth.activate.user.form', $this->token))
                ->line(__('If you did not request, no further action is required.'));
