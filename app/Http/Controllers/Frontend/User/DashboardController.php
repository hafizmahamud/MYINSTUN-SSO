...
class DashboardController extends Controller
{
    ...
    public function profile()
    {
      $token = '';
       ...
       if (!Request::hasCookie('id_token_hint')) {
         //print_r('ooo\ndsdsadasd');
         return redirect()->away('http://id.plisca.net');
       }
      $user = Auth::user();
      $nama = DB::table('users')->where('username', $user->username)->first();
      $role = DB::table('model_has_roles')->where('model_id', $user->id)->first();
    ...
    public function updateProfile(UpdateProfileRequest $request)
    {
      ...
      $nama = DB::table('users')->where('username', $user->username)->first();
      $role = DB::table('model_has_roles')->where('model_id', $user->id)->first();
      $checkEmail = DB::table('users')->where('email', $request->account_email)->first();
      if($checkEmail){
       	if($checkEmail->id != $nama->id)
       	{
         	return redirect()->route('frontend.user.profile')->withFlashDanger(__('E-mel sudah diguna!'));
       	}
      }
      ...
      if($upd)
      {
        DB::table('users')->where('username', $user->username)->update(['display_name' => strtoupper($request->full_name),
        'jabatan' => strtoupper($request->jabatan),
        'no_tel_jabatan' => $request->notel_jabatan,
        'email' => $request->account_email]);
        return redirect()->route('frontend.user.dashboard')->withFlashSuccess(__('Profile is Updated'));
      } else {
	...
	'katalaluan' => 'required',
        ...
        'account_email.unique' => 'E-mel sudah ada!'
      ]);
      $connection = Container::getConnection('default');
      $upd = true;
	$dn = 'inum='.($request->nokp).',ou=people,o=gluu';
      try {
    	$connection->connect();
     	$query = $connection->query();
        $user = new User([
            'cn' => $request->full_name,
            'inum' => strval($request->nokp),
            'displayName' => strtoupper($request->full_name),
            'o' => strtoupper($request->jabatan),
            'telephoneNumber' => $request->notel_jabatan,
            'mail' => $request->account_email,
         ...
         $user->setDn('inum='.strval($request->nokp).',ou=people,o=gluu');
      ...
      { 
        ...
        DB::table('users')->insert([
          'email' =>$request->account_email,
          'display_name' => strtoupper($request->full_name),
        ...
        $myinstun = 'https://myinstun.instun.gov.my/auth/espek/callback';
        return redirect()->away($myinstun);
      } else {
    ...
    public function update(RequestForm $request)
    {
      ...
      if ($upd == true)
      {
        ...
        return redirect()->route('frontend.user.profilepassword')->withFlashDanger(__($error->getErrorMessage()));
      }
