...
{{ html()->modelForm($user, 'PATCH', route('admin.auth.user.update', $user->id))->class('form-horizontal')->open() }}
    <div class="card">
		...
		<div class="form-group row">
                        {{ html()->label(__('IC Number'))->class('col-md-2 form-control-label')->for('username') }}
		            ...
		            <div class="form-group row">
                        {{ html()->label(__('Full Name'))->class('col-md-2 form-control-label')->for('display_name') }}

                        <div class="col-md-10">
                            {{ html()->text('display_name')
                                ->class('form-control')
                                ->placeholder(__('Nama Penuh'))
                                ->attribute('maxlength', 191)
                              }}
                        </div><!--col-->
                    </div><!--form-group-->
			...
			<div class="form-group row">
                        {{ html()->label('Gluu SSO User Type')->class('col-md-2 form-control-label') }}
                                    ...
                                    <tr>
                                        ...
                                        <div class="card">
                                                        <div class="card-header">
                                                            <div class="checkbox d-flex align-items-center">
                                                            {{ html()->label(
                                                                html()->checkbox('usertype',$user->user_type)
                                                                ->class('switch-input')
                                                                . '<span class="switch-slider" data-checked="on" data-unchecked="off"></span>')
                                                                ->class('switch switch-label switch-pill switch-primary mr-2')
                                                            }}
                                                            {{ html()->label('Kakitangan ') }}
                                                           <!--<input type="checkbox" name="usertype" class="switch switch-label switch-pill" value="" {{ ($user->usertype == 1 ? 'checked' : '')}}> Kakitangan -->
                                                        </div>
                                                        </div>
                                        </div>                
                                        </td>
